...
import six.moves
from applib import sh, log, textui, _cmdln as cmdln, _simpledb 
from applib.misc import require_option, xjoin

from pypm.common import supported, net, python
from pypm.common.util import (
    url_join, dlocked, BareDateTime, ConfigParserNamedLists, path_to_url)
from pypm.common.net import URLGetter
from pypm.common.package import BinaryPackage, RepoPackage, PackageFile
...
class MultiRepositorySet(object):
    ...
    def _init(self):
        for name, repository_locations in self.mrc.namedlists.mapping.items():
            ...
            self.reposets[name] = RepositorySet(
                join(self.path, name), name, repository_location.location)
...
class RemoteRepository(object):
    ...
    def get_unique_id(self):
        ...
        return hashlib.md5(self.url.encode('utf8')).hexdigest()
        ...
        with self.bootstrapped():
            ...
            for path in paths:
                for repo in mreposet:
                    ...
                    LOG.error('Not a path in repository: %s', path)
    ...
    def do_genidx(self, subcmd, opts):
        ...
        with self.bootstrapped():
            if self.options.multi_repository_set_path:
                self.genidx_mreposet(opts.filter)
            else:
