...
from pypm.common.repository import RemoteRepositorySet, MultiRepositoryConfig
from pypm.common.package import RepoPackage, BinaryPackage, PackageFile
from pypm.common.util import concise_path, path_to_url, wrapped
from pypm.client.depgraph import req_name, req2str  # utils
...
def install_local_file(pypmenv, pypmfile): # nodeps = True (always!)
    ...
    future
    """
    assert pypmfile.endswith(BinaryPackage.EXTENSION) and exists(pypmfile)
...
def install_remote_file(pypmenv, url):
    ...
    local_file = net.download_file(url, DOWNLOAD_CACHE)
...
class Commands(log.LogawareCmdln):
    ...
    def do_install(self, subcmd, opts, *names):
        ...
        with self.bootstrapped():
            ...
            if len(names) == 1:
                ...
                elif names[0].endswith(BinaryPackage.EXTENSION) and exists(names[0]):
                    return install_local_file(self.pypmenv, names[0])

            install(self.pypmenv, names, nodeps=opts.nodeps, skip_missing=True)
    ...
    def do_uninstall(self, subcmd, opts, *names):
        ...
        with self.bootstrapped():
            ...
            uninstall(self.pypmenv, names, nodeps=opts.nodeps)
...
def add_pep370_dir_on_users_consent():
    ...
    pathenv = [abspath(x.strip()) for x in os.environ.get('PATH', '').split(':')]
    binpath = abspath(join(site.USER_BASE, 'bin'))
    ...
    sh.mkdirs(dirname(firstrun_file))
